...
import com.bird.core.PropertyContainer;
...
public class NodeToStringTransformer {
    ...
    public static String substituteVariable(String input, PropertyContainer pc0,
                                            PropertyContainer pc1) throws ScanException {
    ...
    private void compileNode(Node inputNode, StringBuilder stringBuilder,
                             Stack<Node> cycleCheckStack) throws ScanException {
        Node n = inputNode;
        while (n != null) {
            switch (n.type) {
                case LITERAL:
    ...
    /**
     * Determine if a node has already been visited already by checking the cycleDetectionStack for it's existence. This
     * method is used -- rather than Stack.contains() -- because we want to ignore the Node's 'next' attribute when
